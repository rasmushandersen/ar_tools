...
//     ROS_INFO ("\tMarker Pattern Filename: %s", pattern_filename_);
  ...
  sub_ = n_.subscribe(cameraInfoTopic_,
                      1,
                      &ARSinglePublisher::camInfoCallback,
                      this);
  getCamInfo_ = false;
  ...
  {
    rvizMarkerPub_ = n_.advertise<visualization_msgs::Marker>("visualization_marker",
                                                              0);
  }
}
    ...
    const sensor_msgs::CameraInfoConstPtr & cam_info)
{
  if (!getCamInfo_)
  {
    ...
    cam_sub_ = it_.subscribe(cameraImageTopic_,
                             1,
                             &ARSinglePublisher::getTransformationCallback,
                             this);
    getCamInfo_ = true;
  }
}
    ...
    const sensor_msgs::ImageConstPtr & image_msg)
{
  ...
  try
  {
    capture_ = cv_bridge::toCvCopy(image_msg,
  ...
  if (k != -1)
  {
    ...
    ROS_DEBUG(" QUAT: Pos x: %3.5f  y: %3.5f  z: %3.5f",
              pos[0],
              pos[1],
              pos[2]);
    ROS_DEBUG("     Quat qx: %3.5f qy: %3.5f qz: %3.5f qw: %3.5f",
              quat[0],
              quat[1],
              quat[2],
              quat[3]);
...
// DEPRECATED: Fuerte support ends when Hydro is released
    ...
    if (publishTf_)
    {
      ...
      {
        tf::StampedTransform markerToCam(t.inverse(),
      ...
      {
        tf::StampedTransform camToMarker(t,
                                         image_msg->header.stamp,
                                         image_msg->header.frame_id,
                                         markerFrame_.c_str());
